...
@section('title', 'Cây ATM ' . $atm->district->name . ', ' . $atm->province->name . ' | Cây ATM ' . $atm->name . ' ngân hàng ' . $atm->bank->name_en . ' tại ' . $atm->province->name)
			...
			<div class="col-lg-8">
				...
				<div class="general-info">
					...
					<center>
						...
						<a target="_blank"
							href="http://maps.google.com/maps?q={{ $atm->address }}&spn=,&hl=visssssss">
				...
				<div class="blog-sidebar">
					<div class="widget">
                        ...
                        <div class="social">
                            <ul class="list-unstyled">
                                <li>
                                    <a href="{{route('exchange-rate', ['bank' => str_slug($atm->bank->name_en), 'id' => $atm->bank->id])}}"> » Xem tỷ giá</a>
                                </li>
                                <li>
                                    <a href="{{route('interest-rate', ['bank' => str_slug($atm->bank->name_en), 'id' => $atm->bank->id])}}"> » Xem lãi suất</a>
                                </li>
                                <li>
                                    <a href="{{route('bank', ['bank' => str_slug($atm->bank->slug)])}}"> » Xem chi nhánh</a>
                                </li>
                                <li>
                                    <a href="{{route('bank-atm', ['bank' => str_slug($atm->bank->name_en), 'id' => $atm->bank->id])}}"> » Tra cứu ATM</a>
                                </li>
                                <li>
                                    <a href="{{ route('news-detail', ['slug' => $viewShare['swift_code']->slug, 'id' => $viewShare['swift_code']->id]) }}"> » Mã Swift Code</a>
                                </li>
                                <li>
                                    <a href="{{route('bank-intro', ['bank' => str_slug($atm->bank->name_en), 'id' => $atm->bank->id])}}"> » Giới thiệu</a>
                                </li>
                            </ul>
                        </div>
                    </div>
