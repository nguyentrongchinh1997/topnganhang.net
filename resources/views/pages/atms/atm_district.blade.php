...
@section('title', 'Danh sách cây ATM tại ' . $district->name . '-' . $province->name . ' | Điểm đặt cây ATM tại ' . $district->name . ' - ' . $province->name)

@section('description', 'Tổng hợp điểm đặt ATM tại ' . $district->name . '-' . $province->name . ', danh sách cây rút tiền ATM tại ' . $district->name . ' - ' . $province->name)
			...
			<div class="col-lg-12">
				<ol class="breadcrumb mb-0 p-0">
					...
					<li class="breadcrumb-item active">
						<a href="{{ url()->current() }}">
							<span>
								Điểm đặt cây ATM ngân hàng {{ $bank->name_en }} tại {{$district->name}} - {{$province->name}}
							</span>
						</a>
					</li>
				</ol>
